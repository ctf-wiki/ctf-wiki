...
base xx 中的 xx 表示的是采用多少个字符进行编码，比如说 base64 就是采用以下 64 个字符编码，由于 2 的 6 次方等于 64，所以每 6 个比特为一个单元，对应某个可打印字符。Base64编码后数据bit大小变成原来的4/3,也即3个字符用4个可见字符来表示。1个字符为1字节，3个字节就有 24 个比特，对应于 4 个 Base64 单元，即 3 个字节需要用 4 个可打印字符来表示。它可用来作为电子邮件的传输编码。在 Base64 中的可打印字符包括字母 A-Z、a-z、数字 0-9，这样共有 62 个字符，此外两个可打印符号在不同的系统中而不同。
...
- 与base64类似，base32使用32个可见字符来表示一个二进制数组，编码后数据bit大小变成原来的8/5,也即5个字符用8个可见字符表示，但是最后如果不足5个字符，则会在前面不足5bit的bit数据用0补足了5bit之后全部使用"="补充，直到补足5个字符。由此可知，base32最多有6个等号("=")出现
