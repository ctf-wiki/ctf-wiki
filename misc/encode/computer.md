...
- 与 base64 类似，base32 使用 32 个可见字符进行编码，2 的 5 次方为 32，所以每 5 bit 为 1 个单元。5 字节为 40 bit，对应于 8 个 base32 单元，即 5 个字节用 8 个可打印字符来表示。但如果不足 5 个字节，则会先对第一个不足 5 bit 的数据用 0 补足了 5 bit 后对后面剩余空字节全部使用“=”补充，直到补满 5 个字节。由此可知，base32 最多只有 6 个等号出现。例如：
