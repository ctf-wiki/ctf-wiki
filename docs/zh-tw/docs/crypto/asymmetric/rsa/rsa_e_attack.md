...
攻擊者可以從小到大枚舉 $k$，依次開三次根，直到開出整數爲止。

### 範例

這裏我們以 XMan 一期夏令營課堂練習爲例進行介紹（Jarvis OJ 有復現），附件中有一個 `flag.enc` 和 `pubkey.pem`，很明顯是密文和公鑰了，先用 `openssl` 讀一下公鑰。
...
看到 $e=3$，很明顯是小公鑰指數攻擊了。這裏我們使用 Crypto 庫來讀取公鑰，使用 multiprocessing 來加快破解速度。
...
注意：如果 $p \equiv q \equiv 3 \pmod 4$，則
...
而一般情況下，$p \equiv q \equiv 3 \pmod 4$ 是滿足的，對於不滿足的情況下，請參考相應的算法解決。

### 例子

這裏我們以 XMan 一期夏令營課堂練習（Jarvis OJ 有復現）爲例，讀一下公鑰。
...
$e=2$，考慮 Rabin 算法。首先我們先分解一下 p 和 q，得到
