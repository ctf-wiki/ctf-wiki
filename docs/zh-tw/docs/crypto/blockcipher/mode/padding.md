...
常見的 [填充規則]( https://www.di-mgt.com.au/cryptopad.html) 如下。**需要注意的是，即使消息的長度是塊大小的整數倍，仍然需要填充。**
...
1. 選擇 choice2，追加 `256-33 =223`字節，使當前 flag 不需要填充，追加的最後一個字節設置成 `chr(256-32)`。

2. 服務器對 flag 追加我們的信息，並進行 s 盒替換，結果賦給類中的 flag 變量。

3. 我們再次選擇 choice2，這裏由於我們需要追加，服務器會將類中的 flag 變量取出進行逆 S 盒替換和 unpad，這樣按照這個 unpad 算法會把後面 224 字節的全部當成 padding去掉，明文剩下了真正 flag 的前32位。

4. 我們此時輸入一個字符 i,那麼此時加密的對象就是 `flag[:32]+i`。

5. 選擇 choice1 對當前 flag 加密，控制 i 進行爆破，如果得到的密文和最初的 flag 加密的密文一樣，就得到了 flag 的最後一個字節。

6. 逐字節爆破，直至獲取全部的 flag。 
