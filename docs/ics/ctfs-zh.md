...
这是一道关于MMS协议分析的题目，当拿到文件时使用Wireshark打开文件（有的需要使用tcpdump打开）
...
这类题目的做法一般有两种1.是题目代码模拟运行，然后直接查看相应位置的值，但是一般会遇到需要多错误很难直接运行；2.直接读，对于熟悉梯形图语言的人来说非常简单。
