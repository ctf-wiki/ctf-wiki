...
*如果一些 内敛函数在__builtins__删除 ，我们可以通过`reload(__builtins__)`重新载入获取一个完整的__builtins__*
##### 创建对象，以及引用
...
```
第一个地址是system的偏移，第二个是fopen的偏移，我们可以通过**objdump**获取相关信息
...
#### 0x03
间接引用
在不断的dir过程中，发现__closure__ 这个object保存了参数，可以引用原生的__import__
```py
