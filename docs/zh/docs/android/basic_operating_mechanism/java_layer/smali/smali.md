...
Android Java/Kotlin 代码最终会被编译为 dex 字节码，由 Dalvik 或 ART 运行时加载执行。ART 会根据实际情况采用解释执行、JIT 编译或 AOT 编译等方式运行这些字节码。Android 4.x 及以前主要是 Dalvik VM，Android 5.0 以后主要是 ART（Android Runtime），两者都执行 .dex 中的 Dalvik 字节码格式，但执行方式不同：

- Dalvik 时代：主要依赖解释执行（Interpreter）配合 JIT（Just-In-Time）编译
- ART 时代：早期大量采用 AOT（Ahead-Of-Time）编译，现在又结合 AOT、JIT、Interpreter 混合执行

由于 DEX 字节码可读性较差，逆向工程领域开发了一些其他的形式进行表示，目前通常使用 smali 语法作为其汇编表示形式。借助 apktool、baksmali 等工具，研究人员可以将 dex 文件反汇编为多个 smali 文件进行分析。一般情况下，一个 smali 文件对应一个 Java 类（包括内部类、匿名类等生成的独立类文件）。

Dalvik/ART 采用基于寄存器的虚拟机架构，因此 smali 指令主要围绕虚拟寄存器展开。需要注意的是，这些寄存器并非真实 CPU 寄存器，运行时可能被映射到 CPU 寄存器、栈帧或其他内存位置，具体实现由运行时系统决定。
...
在 Smali 中，registers_size（通常写作指令 .registers）用于声明当前方法所需的寄存器总数。`registers_size` 是 16 位无符号“数量”。最大数量是 65535，一个 `code_item` 的 `registers_size` 最大可表示 65535 个寄存器，即索引 `v0..v65534`。Dalvik 虚拟机在调用该方法时，会根据这个数值为该方法分配对应的栈空间。
...
| 函数参数 | $v_m,v_{m+1},...,v_{m+n-1}$ | $p_0,p_1,...,p_{n-1}$ |
...
.parameter 的个数与方法的使用的参数格式一致，每一条语句声明一个参数。如果方法是非静态方法的话，我们会使用 p0 来表示this，即当前对象，否则的话，参数就正常用 p0 开始。
...
目前共有 5 种比较指令，占据 2d 到 31 之间的操作码。所有这 5 种比较指令的的助记符结构均为：

`cmpkind vAA, vBB, vCC`

其中`A`是存放结果的目标寄存器（8 位），`B`是第一个源寄存器或寄存器对；`C`是第二个源寄存器或寄存器对。

这 5 种比较指令如下：

| op   | 指令                  | 注释                 |
| ---- | --------------------- | -------------------- |
| 2d   | cmpl-float (lt bias)  | 比较两个单精度浮点数 |
| 2e   | cmpg-float (gt bias)  | 比较两个单精度浮点数 |
| 2f   | cmpl-double (lt bias) | 比较两个双精度浮点数 |
| 30   | cmpg-double (gt bias) | 比较两个双精度浮点数 |
| 31   | cmp-long              | 比较两个长整型数     |

lt bias 和 gt bias 只有涉及到 NaN 值的比较时才有区别。在不涉及 NaN 值的情况下，上述五个指令的执行逻辑均为：

**如果 vBB 寄存器大于 vCC 寄存器，则结果为 1，相等则结果为 0，小于则结果为 -1。**

而如果比较的值中有一个是 `NaN` ，那么，“gt bias”指令返回 `1`，而“lt bias”指令返回 `-1`。为什么需要为`NaN` 特意分配两种类型的比较指令呢？这是为了把`NaN`这样的非法值剔除出 true 分支。

例如，检查浮点数是否满足条件 `x < y`时，建议使用`cmpg-float` 指令来进行。因为如果结果是 `-1`，则表示测试结果为 true，其他值表示测试结果为 false。false 的情况包括“当前比较有效但是x >= y”，或“其中一个值是 `NaN`”
