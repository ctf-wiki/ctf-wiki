...
可变长度的类型其实都是基于 LEB128(Little-Endian Base 128) 类型的，可以用于表示 32 位大小的 int 数字，其根据所要表示的数字的大小来选择合适的长度。如下图所示，其中每个字节的最高位表示是否使用下一个字节，1 表示使用，0 表示不使用。故而每个字节其实只有 7 个有效的 bit 位用来表示相应的数字。如果有一个 LEB128 类型的变量使用了 5 个字节，并且第五个字节的最高位为 1 ，那说明出现了问题。
...
| 名称              | 格式                        | 说明                                       |
| --------------- | ------------------------- | ---------------------------------------- |
| magic           | ubyte[8] = DEX_FILE_MAGIC | 标识DEX文件，随 Android 演进，常见有 035、037、038、039、040，例如 DEX_FILE_MAGIC ="dex\n035\0"   |
...
| header_size     | uint              | 文件头的大小，以字节为单位，随版本进行变化，v40 及以前为 `0x70`，v41 及以后为 `0x78`。                           |
