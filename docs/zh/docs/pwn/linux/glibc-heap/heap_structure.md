...
**关于使用负数的解释**:此处最终目的是确保16位对齐, 已知目前已有的内容是6个指针空间, 为了确保16位对齐, 我们需要一个padding, 大小为16 * x - 6 * SIZE_SZ (其中x为自然数, 在确定的时候选择一个最合适的自然数x即可), 
前置知识:

- （-11）, -3和5, 13 四个数分别关于8取模的结果是一样的;
- 对一个0 <= y < 模 的整数多次求同一个模，结果不变，恒为y。
- 

	  padding
   
= 16 * x - 6 * SIZE_SZ  # 我们的目的是padding的结果为0 <= padding < 16 的整数
